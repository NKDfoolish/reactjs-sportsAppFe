...
const ManageTypeShip = () => {
    ...
    let handleDeleteTypeVoucher = async (id) => {
        ...
        if (res && res.errCode === 0) {
            toast.success("Deleted voucher type successfully")
            let arrData = await getAllTypeVoucher({
                limit: PAGINATION.pagerow,
        ...
        } else toast.error("Fail to delete voucher type")
    }
    ...
    let handleOnClickExport = async () => {
        ...
        if (res && res.errCode == 0) {
            await CommonUtils.exportExcel(res.data, "List voucher type", "ListTypeVoucher")
        }

    }
    return (
        <div className="container-fluid px-4">
            <h1 className="mt-4">Manage voucher</h1>
